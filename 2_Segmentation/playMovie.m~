function [newSeeds,maskStructure0,rgb0,maskStack0]=playMovie( imageRef, stack, maskStructure, maskStack)
...
bigStack = uint16(zeros(512,2333,stackLen));
i = 1;
while i <= stackLen
    bigStack(:,:,i) = uint16(ImageBiggerer(stack(:,:,i)));
...
function init_ui()    
    ...
    for sindex=1:size(maskStructure0,2)
        ...
        %circles(xCent-imS/2:xCent+imS/2-1, yCent-imS/2:yCent+imS/2-1, color)=...
        % uint8(rgb0(xCent-imS/2:xCent+imS/2-1, yCent-imS/2:yCent+imS/2-1, color))...
        %  +uint8(colorMult*maskStack0(:,:,sindex));
    end
    %h=imshow((rgb0+uint8(circles)),'Border','tight');
    h=imshow((rgb0),'Border','tight');
    displayIntructions()
...
function displayIntructions()
    ...
    txt8 = 'Right Click for Deletion (Square all the cells you want to delete';
