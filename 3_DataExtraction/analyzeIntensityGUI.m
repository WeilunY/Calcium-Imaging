...
%%%[FOR THE LAST CELL THIS CODE RUNS IMPROPERLY FOR SOME REASON]
    ...
    % mean quality factor
    for i=iStart : length(firedNeurons)
        avgi = mean(intensityData2(firedNeurons(i),:));
    ...
    fprintf(fileID, ['Average Quality Factor (SD/AVGI): ', num2str(avgQualityFactor), '\n']); 
...
function Intensity_DeltaF_Plotter()
    ...
    for i=iStart:iEnd
        ...
        txt_intensity = ['AVGI: ' num2str(round(avgi, 3))];
        text(10, (inc-1)+ 0.6, txt_intensity, 'color', 'r');
        
        % add neg std for each cell
        txt_std = ['SD: ' num2str(round(sd, 3))];
